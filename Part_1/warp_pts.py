...
def warp_pts(X, Y, interior_pts):
    ...
    # print(H.shape)
    ##### STUDENT CODE START #####
    x = np.ones((interior_pts.shape[0], 1))
    interior_pts_x = np.hstack((interior_pts, x))
    warped_pts = interior_pts_x @ H.T
    # print(warped_pts.shape)
    y = warped_pts[ : , : 2]
    z = warped_pts[ : , -1].reshape((-1, 1))
    warped_pts = np.divide(y, z)
    ##### STUDENT CODE END #####
